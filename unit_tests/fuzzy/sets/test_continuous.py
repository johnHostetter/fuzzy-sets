...
class TestGaussian(unittest.TestCase):
    ...
    def test_multi_input_with_centers_given(self) -> None:
        ...
        # make sure the Gaussian parameters are still identical afterward
        assert torch.isclose(gaussian_mf.sigmas.cpu(), torch.tensor(sigmas)).all()
        assert torch.isclose(gaussian_mf.centers.cpu(), torch.tensor(centers)).all()
        # the outputs of the PyTorch and Numpy versions should be approx. equal
        assert np.isclose(
...
class TestTriangular(unittest.TestCase):
    ...
    def test_multi_input(self) -> None:
        ...
        # make sure the Gaussian parameters are still identical afterward
        assert torch.isclose(triangular_mf.centers.cpu(), torch.tensor(centers)).all()
        assert torch.isclose(triangular_mf.widths.cpu(), torch.tensor(widths)).all()
        # the outputs of the PyTorch and Numpy versions should be approx. equal
        assert np.isclose(
